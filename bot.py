...
async def create_order(request: Request):
    ...
    receipt = data.get('receipt', None)  # base64 image for guests

    is_guest = office.startswith("زائر")
    # Guests: auto-approved & paid; Office users: pending
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO orders (user_id, details, total_price, location, timestamp, status, is_paid, order_type, receipt, approved_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
...
async def sync_user(office: str):
    ...
    try:
        ...
        total_debt = sum(o["total_price"] for o in orders if o["is_paid"] == 0)

        # Check if review is pending (approved 10+ min ago, not reviewed)
        review_due = None
        if not is_guest:
            c.execute(
            ...
            if rev_row:
                try:
                    ...
                    if (now - approved_time).total_seconds() >= 600:  # 10 minutes
                        review_due = {
                            "order_id": rev_row[0],
                            "details": rev_row[1],
...
async def admin_dashboard():
    try:
        ...
        # Active orders (pending/missing)
        c.execute("SELECT id, details, total_price, location, status, order_type, missing_note FROM orders WHERE status IN ('انتظار','صنف_ناقص') ORDER BY id ASC")
        active_rows = c.fetchall()
        active_orders = [
            {"id": r[0], "details": r[1], "total_price": r[2], "location": r[3],
             "status": r[4], "order_type": r[5], "missing_note": r[6]}
